...
class service2ssh extends service2smb {
    ...
    public function ssh2enum($dico){
        ...
        $check_user = trim($this->req_ret_str("python $this->dir_tools/ssh_enum.py -p $this->port $this->ip user_doesnt_exist_from_me 2> /dev/null | grep 'is a valid username' | sed \"s/is a valid username//g\" "));
        if (!empty($check_user)) {
        ...
        foreach ($users_test as $user_test){
           ...
           $check_user = trim($this->req_ret_str("python $this->dir_tools/ssh_enum.py -p $this->port $this->ip $user_test 2> /dev/null | grep 'is a valid username' | sed \"s/is a valid username//g\" "));
