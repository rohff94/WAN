...
Only root can forward privileged ports. Dynamic port forwardings can also be specified in the configuration file.
    ...
    public function __construct($stream,$eth,$domain,$ip,$port,$protocol) {
        ...
        $data = "iptables -L -n";
       // $this->firewall_rules_str = $this->req_str($stream,$data,$this->stream_timeout,"");
    ...
    public function lan2pivot($stream){
...
This is one way to find out IP addresses for other internal servers.");
    ...
    public function lan2pivot4static_tools($stream,$remote_network){
