...
mysql> select id,ip from IP WHERE ip IN (SELECT ip FROM IP GROUP BY ip HAVING COUNT(*) > 1) ORDER BY ip;
	...
	if ($this->mysql_ressource == FALSE) {$this->log2error("Connexion to Mysql $this->mysql_database Failled",__FILE__,__CLASS__,__FUNCTION__,__LINE__,"","");exit();}
...
public function ip2geo($ip){
    $ip = trim($ip);
    if (!empty($ip)){
        if ($this->ip4priv($ip)) return "Local IP";
        if (!$this->ip4priv($ip) ){
